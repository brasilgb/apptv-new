...
export const getDataFaturamento = async () => {
  ...
  const file: any = await fs.readFile('/mnt/apptv/PainelTV/faturamento-painel-tv.json');
...
export const getDataGrafico = async () => {
  ...
  const file = await fs.readFile('/mnt/apptv/PainelTV/grafico-painel-tv.json', 'utf8');
  const data = JSON.parse(file);
  return data;
};
